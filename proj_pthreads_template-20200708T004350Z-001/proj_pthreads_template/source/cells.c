...
size_t countLiveNeighbours(size_t row, size_t col)
{
	...
	// your code goes here
	/*int a=env[row-1][col-1];
	 int b=env[row-1][col];
	 int c=env[row-1][col+1];
	 int d=env[row][col-1];
	 int e=env[row][col+1];
	 int f=env[row+1][col-1];
	 int g=env[row+1][col];
	 int h=env[row+1][col+1];
	 */
	if (*env[a_posn] == 1)
		cell_count++;
	if (*env[b_posn] == 1)
...
void updateCell(size_t r, size_t c)
{
	...
	if (live_neighbours == 3 && state_cell == 0)
	state_cell = 1;
	else (state_cell==1 &&(live_neighbours > 3 || live neighbours< 2 ))
	state_cell = 0;
...
void* updateCommFunc(void *param)
{
	...
	// your code goes here
	size_t r = ((threadID_t*) param)->row;
	size_t c = ((threadID_t*) param)->col;
	updateCell(r, c);
	usleep(32767);
	pthread_exit(NULL);
}
